...
const base=Constants.expoConfig?.extra?.apiBase || "https://api.picklescoop.com/wp-json/lcm-mobile/v1";
...
export async function api(path,options={}){const token=await getToken();const headers={"Content-Type":"application/json",...(token?{Authorization:`Bearer ${token}`}:{ }),...(options.headers||{})};const r=await fetch(base+path,{...options,headers});const text=await r.text();let data={};try{data=text?JSON.parse(text):{}}catch{data={message:text}}if(!r.ok){const e=new Error(data?.message||`Request failed (${r.status})`);e.status=r.status;throw e}return data}
